...
let dapat = (Math.floor(Math.random() * 500))
let nomors = m.sender
  let who
  ...
  if (100 > users[who].limit) throw 'Target Gaada Limit Pak'
  users[who].limit -= dapat * 1
  users[m.sender].limit += dapat * 1
  global.db.data.users[m.sender].lastmit = new Date * 1
  conn.reply(m.chat, `Berhasil Merampok limit Target Sebesar ${dapat}`, m)

}else conn.reply(m.chat, `Anda Sudah Curi Limit dan berhasil sembunyi , tunggu ${timers} untuk mencuri lagi`, m)
}
