...
let handler = async (m, { conn, args, usedPrefix, command }) => {
    try {
        ...
        if (/pelan/.test(command)) set = '-filter:a "atempo=0.7,asetrate=44100"'
...
handler.help = ['bass', 'blown', 'deep', 'earrape', 'fast', 'fat', 'nightcore', 'reverse', 'robot', 'pelan', 'smooth', 'tupai', 'vibra'].map(v => v + ' [vn]')
handler.tags = ['audio']
handler.command = /^(bass|blown|deep|earrape|fas?t|nightcore|reverse|robot|pelan|smooth|tupai|squirrel|chipmunk|vibra)$/i
handler.limit = true
