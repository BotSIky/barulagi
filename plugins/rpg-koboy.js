let handler = (m, { conn, usedPrefix, command, text }) => {
  conn.tembak = conn.tembak || { musuh: [], tembak: [] }
   if(/kiri/i.test(text)) {
    ...
    return conn.send2But(m.chat, pos, "©LitRHap", "Kiri", `${usedPrefix}${command} kiri`, "Kanan", `${usedPrefix}${command} kanan`)
    ...
    if(conn.tembak.musuh.indexOf("🥷") === conn.tembak.tembak.indexOf("🤠")) return conn.sendBut(m.chat, pos, "©LitRHap", "Tembak", `${usedPrefix}${command} tembak`)
    return conn.send2But(m.chat, pos, "©Kyyy", "Kiri", `${usedPrefix}${command} kiri`, "Kanan", `${usedPrefix}${command} kanan`)
  }
}
handler.help = ['koboy']
handler.tags = ['rpg']
